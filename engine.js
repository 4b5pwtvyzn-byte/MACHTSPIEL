...
function passLaw(law) {
  state.player.passedLaws.push(law);
  state.player.lawsPassed = (state.player.lawsPassed || 0) + 1;
  state.player.pendingLaw = null;

  // Effekte anwenden
  (law.effects || []).forEach(eff => {
    if (eff.group && state.player.voterApproval[eff.group] !== undefined) {
      state.player.voterApproval[eff.group] = clamp(state.player.voterApproval[eff.group] + eff.value, 0, 60);
    }
    if (eff.stat === 'media')    state.player.media = clamp(state.player.media + eff.value, 0, 100);
    if (eff.stat === 'budget')   state.player.budget.balance += eff.value;
    if (eff.stat === 'credibility') state.player.credibility = clamp(state.player.credibility + eff.value, 0, 100);
    if (eff.stat === 'coalition') state.player.coalitionSatisfaction = clamp(state.player.coalitionSatisfaction + eff.value, 0, 100);
  });
...
function failLaw(law) {
  state.player.failedLaws.push(law);
  state.player.pendingLaw = null;
  state.player.media = clamp(state.player.media - 3, 0, 100);
        ...
        <div class="decision-options">
          ${available.map(law => `
            <button class="decision-option" data-propose-law="${law.id}">
              ...
              <div class="decision-option__content">
                <div class="decision-option__title">${law.title}</div>
                <div class="decision-option__desc">${law.description || ''}</div>
                ${law.requiresBundesrat ? '<div class="text-xs mt-1" style="color:var(--color-warning)">Bundesratspflichtig</div>' : ''}
              </div>
            </button>
          `).join('')}
        </div>
...
function showSaveModal(code) {
  ...
  overlay.innerHTML = `
    <div class="save-load-modal">
      ...
      <div class="save-load-modal__desc">Kopieren Sie den Code und bewahren Sie ihn sicher auf.</div>
